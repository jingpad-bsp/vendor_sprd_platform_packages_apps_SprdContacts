...
public class AccountFilterUtil {
    ...
    public static void handleAccountFilterResult(
        ...
        if (resultCode == Activity.RESULT_OK) {
            ...
            if (filter.filterType == ContactListFilter.FILTER_TYPE_CUSTOM) {
                //SPRD: Bug693204 Display contacts only with phone number.
                filterController.selectCustomFilter(filter);
            } else {
    ...
    public static String getActionBarTitleForFilter(Context context, ContactListFilter filter) {
        ...
        // SPRD: add for bug 474264,693201 contacts count display feature
            ...
            return context.getString(R.string.title_from_custom);
        // UNISOC: Add for Bug1389085
        } else if (filter.filterType == ContactListFilter.FILTER_TYPE_STARRED) {
            return context.getString(R.string.title_starred_contacts);
        }
    ...
    private static String getActionBarTitleForAccount(Context context, ContactListFilter filter) {
        ...
        String accountName = info.getNameLabel().toString();
        if (accountName != null && accountName.equals("Phone")) {
            accountName = context.getString(R.string.label_phone);
        }
        return context.getString(R.string.title_from_other_accounts, accountName);
    }
}
