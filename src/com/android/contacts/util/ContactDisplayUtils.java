...
public class ContactDisplayUtils {
    ...
    /** UNISOC: add for bug1029228/1036195/1029228/1059308, Adapt to Arabic. @{ */
    public static boolean needChangeLTR(String name) {
        if (!isRTL()) {
            return false;
        }

        return Pattern.compile("^[a-zA-Z0-9\\W]+$").matcher(name).matches();
    }
    ...
    public static String getFormatDisplayName(String displayName) {
        if (displayName != null && Patterns.PHONE.matcher(displayName).matches()) {
            displayName = "\u202A" + displayName + "\u202C";
        }
        return displayName;
    }
    /** UNISOC: add for bug1084337, Adapt to Arabic for vcf import Toast . @{ */
    public static String getFormatToastName(String displayName) {
        if (displayName != null) {
            int index = displayName.lastIndexOf(".");
            if (index != -1) {
                String subDisplayname = displayName.substring(0, index);
                if (subDisplayname != null && Patterns.PHONE.matcher(subDisplayname).matches()) {
                    displayName = "\u202A" + displayName + "\u202C";
                }
            }
        }
