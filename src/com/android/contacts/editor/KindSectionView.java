...
public class KindSectionView extends LinearLayout {
    ...
    private class NonNameEditorListener implements Editor.EditorListener {
        ...
        public void onDeleteRequested(Editor editor) {
            /**
             * UNISOC: modify for bug1046611 Quickly click the "x" icon to the right of the two Numbers, and the number field displays an exception * @{
             */
            EditorAnimator.getInstance().endOldAnimation();
            /*
              *@}
             */
    ...
    //UNISOC: add for bug1012855, add for orange_ef anr/aas/sne feature
    private void rebuildPhoneView() {
        final String mimeType = mKindSectionData.getMimeType();
        Log.d("KindSectionView" , "rebuildPhoneView, mimeType = " + mimeType);
        if (Phone.CONTENT_ITEM_TYPE.equals(mimeType) && mEditors != null) {
            for (int i = 0; i < mEditors.getChildCount(); i++) {
                final View view = mEditors.getChildAt(i);
                if (view instanceof Editor && view.findFocus() == null) {
                    view.requestFocus();
                }
            }
        }
    ...
    public void updateEmptyEditors(boolean shouldAnimate) {
        ...
        if (isNameKindSection) {
            ...
            if (editorCount == emptyEditors.size() && mHideIfEmpty) {
                setVisibility(GONE);
                //UNISOC: add for bug1184009, hide softkeyboard when editview has gone
                hideSoftKeyboard();
                return;
            }
            setVisibility(VISIBLE);
    ...
    private void hideSoftKeyboard() {
        ...
        if (inputMethodManager != null) {
            inputMethodManager.hideSoftInputFromWindow(mEditors.getWindowToken(), 0);
        }
    }
