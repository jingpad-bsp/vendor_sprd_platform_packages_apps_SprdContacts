...
public class DataItem implements Collapser.Collapsible<DataItem> {
    ...
    public boolean shouldCollapseWith(DataItem t, Context context) {
        ...
        //UNISOC: add for bug1046829, Don't reset duplicate data items for USIM/SIM.
        boolean result = MoreContactUtils.shouldCollapse(getMimeType(), buildDataString(context, mKind),
                t.getMimeType(), t.buildDataString(context, t.getDataKind()));

        if (result) {
            return !(mIsUsim
                    && Phone.CONTENT_ITEM_TYPE.equals(getMimeType())
                    && RawContactModifier.getTypePrecedence(mKind, getKindTypeColumn(mKind))
                    >
                    RawContactModifier.getTypePrecedence(t.getDataKind(), t.getKindTypeColumn(t.getDataKind())));
        }
    ...
    //UNISOC: add for bug1046829, Don't reset duplicate data items for USIM/SIM.
    public void setUsimAccountType(boolean isUsim) {
        mIsUsim = isUsim;
    }
}
