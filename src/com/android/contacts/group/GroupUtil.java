...
public final class GroupUtil {
    ...
    public static final String ACTION_MOVE_GROUP_COMPLETE = "moveGroupComplete";
    ...
    /**
     * SPRD: Bug693205 Add group feature for Contacts
     * Returns an Intent to move contacts to another group
     * @{
     */
    public static Intent createMoveMemberIntent(
            Context context, GroupMetaData groupMetaData, ArrayList<String> otherGroupIds,
            ArrayList<String> otherGroupTitles, long[] defaultSelection) {
        final Intent intent = new Intent(context, ContactSelectionActivity.class);
        intent.setAction(Intent.ACTION_PICK);
        intent.setType(Groups.CONTENT_TYPE);
        intent.putExtra(UiIntentActions.MOVE_GROUP_METADATA, groupMetaData);
        intent.putExtra(UiIntentActions.TARGET_GROUP_IDS, otherGroupIds);
        intent.putExtra(UiIntentActions.TARGET_GROUP_TITLES, otherGroupTitles);
        intent.putExtra(UiIntentActions.SELECTION_DEFAULT_SELECTION, defaultSelection);
        return intent;
    }
    /**
     ...
     */
    public static boolean isEnableGroup(GroupListItem groupListItem, List<AccountInfo> list) {
        boolean flag = false;
        if (list != null && groupListItem != null) {
            for (AccountInfo a : list) {
                if (a.getAccount().type.equals(groupListItem.getAccountType())) {
                    flag = true;
                    break;
                }
            }
