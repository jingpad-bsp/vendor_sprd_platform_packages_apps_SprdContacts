...
import com.sprd.contacts.ShortCutBroadcastReceiver;
...
*/
public class QuickContactActivity extends ContactsActivity implements SensorEventListener,
...
*/
    ...
    private ShortCutBroadcastReceiver mShortCutBroadcastReceiver;
    ...
    //UNISOC: add for bug715533, Disable email app, Contacts should be prompt "No app was found to handle this action." when click email
    private static final String EMIAL_TYPE_DATA = "vnd.android.cursor.item/email_v2";

    //UNISOC: add for bug1165365, in airplane mode,can not make call
    private static final int SLOT_ID_ONE = 0;
    private static final int SLOT_ID_TWO = 1;
    ...
    //SPRD: add for bug717875 limit the join contacts to 10
    private static final int MAX_JOIN_CONTACTS_NUMBER = 10;
    //SPRD: add for bug950925 Click on the Google account contact and the details page ‘s enteranimation with high probability for three times.
    private boolean mHasAlreadyrunEntranceAnimation = false;
    // UNISOC: add for Bug1397876
    ...
    final OnClickListener mEntryClickHandler = new OnClickListener() {
        ...
        public void onClick(View v) {
            ...
            /**SPRD Bug715533 Disable email app, Contacts should be prompt "No app was found to handle this action." when click email {@*/
            if(intent == null){
                if(v instanceof EntryView){
                    EntryView entryView = (EntryView)v;
                    if(entryView != null){
                        EntryContextMenuInfo entryContextMenuInfo = (EntryContextMenuInfo)entryView.getContextMenuInfo();
                        if(entryContextMenuInfo != null && EMIAL_TYPE_DATA.equals(entryContextMenuInfo.getMimeType())){
            ...
            if (Intent.ACTION_CALL.equals(intent.getAction())) {
                ...
                if (phoneCount > 1) {
                    ...
                    isImsRegistered = TelephonyManager.from(mContext).isImsRegistered();
                }

                final boolean isAirplaneModeOn = Settings.System.getInt(mContext.getContentResolver(),
    ...
    private final OnCreateContextMenuListener mEntryContextMenuListener =
        ...
        public void onCreateContextMenu(ContextMenu menu, View v, ContextMenuInfo menuInfo) {
            ...
            //UNISOC: modidy for bug1165385,Adapt to Arabic for menu text
            final String selectedMimeType = info.getMimeType();
            String copyText = info.getCopyText();
            if (Phone.CONTENT_ITEM_TYPE.equals(selectedMimeType)) {
                copyText = com.android.contacts.util.ContactDisplayUtils.getFormatDisplayName(copyText);
            }
            menu.setHeaderTitle(copyText);
            ...
            /** UNISOC: add for bug1143703, Sim contacts should not support set_default or clear_default. @{ */
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        /**
         * SPRD:Bug718783 add toast while batch operation
         *
         * @{
         */
        ...
        mShortCutBroadcastReceiver = new ShortCutBroadcastReceiver();
        mShouldLog = true;
        ...
        if (Features.supportVideoCallIcon()){
            tryRegisterImsListener();
        }
        /**
         * UNISOC: Bug1347038 Update black list menu item when when a content change occurs.
         * @{
         */
        ...
        getContentResolver().registerContentObserver(BlackColumns.BlackMumber.CONTENT_URI, false,
                mContentObserver);
        /**
         * @}
         */
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        if (deletedOrSplit) {
            ...
            /**
             * SPRD: Bug717875 limit the join contacts to 10 @{
             */
    ...
    protected void onNewIntent(Intent intent) {
        ...
        /**
         * SPRD:Bug794642 Contact list interface, click on any contact twice, the details interface only pops up in half.
         *
         * @{
         */
//        mIsEntranceAnimationFinished = true;
    ...
    private void runEntranceAnimation() {
         ...
         * @{
         */
        if (mHasAlreadyBeenOpened && mIsEntranceAnimationFinished) {
            return;
        }
        /**
         * @}
    ...
    private void bindContactData(final Contact data) {
        /**
         * SPRD:Bug971399 ims change maybe both data and mContactData is null
         *
         * @{
         */
        if (data == null && mContactData == null){
            return ;
        }
        /** @}**/
        ...
        if (mScroller != null) {
            ...
            SchedulingUtils.doOnPreDraw(mScroller, /* drawNextFrame = */ false,
                    new Runnable() {
                        ...
                        public void run() {
                            /**
                             * SPRD:Bug950925 Click on the Google account contact and the details page ‘s enteranimation with high probability for three times.
                             *
                             * @{
                             */
                            if (!mHasAlreadyrunEntranceAnimation) {
                                runEntranceAnimation();
                            }
                            mHasAlreadyrunEntranceAnimation = true;
                            /**
                             * @}
                             */
                        }
    ...
    public void onStart(){
        super.onStart();
        registerReceiver(this.mShortCutBroadcastReceiver,new IntentFilter("com.android.contacts.quickContactShortcutPinned"));
        ShortCutBroadcastReceiver.setContext(this);
    }
    ...
    protected void onResume() {
        super.onResume();
        /**
         * SPRD: Bug750902 Contact photo display blank after setting contact photo in gallery
         * @{
         */
        mHasComputedThemeColor = false;
        /**
         ...
         * @{
         */
        mEasyDialPreference = Settings.Global.getInt(getContentResolver(), Settings.Global.EASY_DIAL, 0) != 0;
    ...
    private void populateContactAndAboutCard(Cp2DataCardModel cp2DataCardModel,
        ...
        if (!TextUtils.isEmpty(customAboutCardName)) {
            ...
            //UNISOC: add for bug1185588, hide title textview of AboutCard when title is null
            mAboutCard.hideTitleTextView();
        }
        ...
        //SPRD: add for bug980200， FDN contacts can not be edited
        if (contactCardEntries.size() == 0 && aboutCardEntries.size() == 0
                && mContactData != null && !mContactData.isFdnContact()) {
        ...
        if (aboutCardEntries.size() > 0) {
            ...
            // UNISOC: Bug1140325 AboutCard is still VISIBLE after deleted.
    ...
    private int getSelectedContactJoinNum(long contactId) {
        ...
        try {
            /**
             * SPRD:bug 725120 mContactData may is null @{
             *
             **/
            if (cursor != null) {
                joinNum = joinNum + cursor.getCount();
            }
            if (mContactData != null && mContactData.createRawContactDeltaList() != null) {
                joinNum = joinNum + mContactData.createRawContactDeltaList().size();
            }
            /**
             * @}
             *
    ...
    private Cp2DataCardModel generateDataModelFromContact(
        ...
        for (RawContact rawContact : data.getRawContacts()) {
            for (DataItem dataItem : rawContact.getDataItems()) {
                ...
                //UNISOC: add for bug1046829, Don't reset duplicate data items for USIM/SIM.
                if (accountType != null && USimAccountType.ACCOUNT_TYPE.equals(accountType.accountType)) {
                    dataItem.setUsimAccountType(true);
                }
        ...
        //UNISOC: add for bug1205524, insert a contacts with aboutCard information but with no name
        if (aboutCardEntries.size() > 0 && TextUtils.isEmpty(aboutCardName.value)) {
            aboutCardName.value = this.getResources().getString(R.string.about_card_title);
        }
    ...
    private static Entry dataItemToEntry(DataItem dataItem, DataItem secondDataItem,
        ...
        if (dataItem instanceof ImDataItem) {
            ...
            if (!TextUtils.isEmpty(phone.getNumber())) {
                ...
                Log.d(TAG,"master "+ "isVideoEnabled"+isVideoEnabled);
                ...
                if (CallUtil.isCallWithSubjectSupported(context)) {
                    ...
                    if ((isPresenceEnabled && isPresent) || !isPresenceEnabled) {
                        ...
                        if (PhoneNumberUtils.isEmergencyNumber(phone.getNumber())){
                            thirdIntent = new Intent(Intent.ACTION_DIAL,Uri.parse("tel:"+phone.getNumber()));
         ...
         */
            ...
            if (contactData != null) {
                for (GroupMetaData groupMetaData : contactData.getGroupMetaData()) {
                    if (groupId == groupMetaData.groupId) {
                        if (!groupMetaData.defaultGroup && !groupMetaData.favorites) {
                            groupTitle = groupMetaData.groupName;
                        }
                        break;
                    }
                }
    ...
    public void onBackPressed() {
        ...
        /**
         * SPRD:828203 When delete or create contacts, need to delete the screenshot of the performance @{
         **/
        //PerformanceManagerInternal.getDefault().removeApplcationSnapShot("com.android.contacts");
        /**
         * @}
         **/
        if (mScroller != null) {
    ...
    protected void onStop() {
        super.onStop();
        if(mShortCutBroadcastReceiver != null){
            unregisterReceiver(mShortCutBroadcastReceiver);
        }
    ...
    public void onDestroy() {
        ...
        ShortCutBroadcastReceiver.setContext(null);
        super.onDestroy();
        /**
         * SPRD:Bug 737067 No phone permisson leading to crash in monkey test
         */
    ...
    public void makeShortcutToast(String name, int type) {
        //UNISOC: add for bug1029228/1036195/1029228, Adapt to Arabic
        if (!TextUtils.isEmpty(name) && com.android.contacts.util.ContactDisplayUtils.needChangeLTR(name)) {
            name = "\u202A" + name + "\u202C";
        }
        switch(type){
            case 1:
                Toast.makeText(QuickContactActivity.this, TextUtils.isEmpty(name)
                    ? getString(R.string.createContactShortcutSuccessful_NoName)
                    : getString(R.string.createContactShortcutSuccessful, name),
                    Toast.LENGTH_SHORT).show();
                break;
            case 2:
                Toast.makeText(QuickContactActivity.this, getString(R.string.createContactShortcutSuccessful, name), Toast.LENGTH_SHORT).show();
                break;
        }
    }
    ...
    private void createLauncherShortcutWithContact() {
        /**
         * SPPRD BUG:748770monkeytest The occasional contact stops running
         * （NullPointerException: Attempt to invoke virtual method 'long com.android.contacts.model.Contact.getId()'
         * on a null object reference）.
         * @{
         */
        if(!isShortcutCreatable() || isFinishing()){
            return;
        }
        /**
         * @}
        ...
        if (BuildCompat.isAtLeastO()) {
            ...
            /** UNISOC: add for bug1059308/1068233, Adapt to Arabic. @{ */
            String displayName = com.android.contacts.util.ContactDisplayUtils.getFormatDisplayName(mContactData.getDisplayName());
            /** @｝ */
            if (displayName == null) {
            ...
            // SPRD: Bug862554,823598,766524 Calling application must have a foreground activity or a foreground service
            List<ShortcutInfo> infoList = shortcutManager.getPinnedShortcuts();
            List<String> infoIdList = new ArrayList<>();
            for (int i = 0; i < infoList.size(); i++) {
                Log.d(TAG, "createLauncherShortcutWithContact: info[" + i + "].getId() = " + infoList.get(i).getId());
                infoIdList.add(infoList.get(i).getId());
                if (infoList.get(i).getId().equals(shortcutInfoId)){
                    makeShortcutToast(displayName,2);
                    return;
                }
            }
            if (shortcutInfo != null) {
                ...
                Intent pinnedShortcutCallbackIntent =
                    new Intent("com.android.contacts.quickContactsShortcutPinned");
                pinnedShortcutCallbackIntent.putExtra("shortcutName",displayName);
                pinnedShortcutCallbackIntent.addFlags(Intent.FLAG_RECEIVER_INCLUDE_BACKGROUND);
                try {
                    shortcutManager.requestPinShortcut(shortcutInfo, PendingIntent.getBroadcast (this,0,pinnedShortcutCallbackIntent,PendingIntent.FLAG_UPDATE_CURRENT).getIntentSender());
                } catch (Exception e) {
                    Log.d(TAG, "Fail to create launcher shortcut, Exception: "+e);
                }
            }
        } else {
    ...
    public boolean onCreateOptionsMenu(Menu menu) {
        ...
        /*SPRD: Bug 708753  Contacts crash when enter people_activity in Guest Mode begin*/
        if (mContactData != null && !mContactData.isUserProfile() && userManager.isSystemUser()) {
            addBlackItem(menu, mContactData);
        }
        /*SPRD: Bug 708753  Contacts crash when enter people_activity in Guest Mode end*/
    ...
    public boolean onPrepareOptionsMenu(Menu menu) {
        if (mContactData != null) {
            ...
            List<AccountWithDataSet> allAccounts = AccountTypeManager.getInstance(mContext).getAccounts(true);
            ...
            if ((accountType != null) && (SimAccountType.ACCOUNT_TYPE.equals(accountType.type) || USimAccountType.ACCOUNT_TYPE
                ...
                shortcutMenuItem.setVisible(isShortcutCreatable());
                sendToVoiceMailMenuItem.setChecked(mSendToVoicemailState);
                sendToVoiceMailMenuItem.setVisible(mOptionsMenuOptions);
                ringToneMenuItem.setVisible(mOptionsMenuOptions);
            }
            /**
             * @}
             **/
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        if (id == R.id.menu_star) {// Make sure there is a contact
            ...
            //UNISOC: bug1050892 contact app open and contact add can be directly added without any permission request
    ...
    private void pickRingtone() {
        ...
        /**
         * SPRD bug: 725253 not allow the user to pick a silent ringtone @{
         **/
        ...
        Uri ringtoneUri = EditorUiUtils.getRingtoneUriFromString(mCustomRingtone,
                CURRENT_API_VERSION);
    ...
    private void dismissProgressBar() {
        //UNISOC: add for bug1161926/1188462, java.lang.IllegalArgumentException
    ...
    public void onSensorChanged(SensorEvent event) {
        ...
        if (mTelephonyManager != null &&
            ...
            try {
                ...
                /* SPRD: Bug1003992 Turn on the sensor dial. When the contact number is an emergency number such as 112, pick up the phone and do not dial  @{ */
    ...
    private void onCopyRequested(final String lookupKey) {
        ...
        SelectAccountDialogFragment.show(getFragmentManager(),
                R.string.copy_to, AccountTypeManager.AccountFilter.CONTACTS_WRITABLE, args);
    }
    ...
    public void onAccountChosen(AccountWithDataSet dstAccount, Bundle extraArgs) {
        ...
        confirmImport(dstAccount, Long.toString(mContactData.getContactId()));
    }
    ...
    private void confirmImport(final AccountWithDataSet dstAccount, final String ids) {
        if (dstAccount != null
            ...
            args.putString(UiIntentActions.TARGET_CONTACT_IDS_EXTRA_KEY, ids);
            ...
            return new AlertDialog.Builder(getActivity())
                    .setPositiveButton(android.R.string.ok, new DialogInterface.OnClickListener() {
                        public void onClick(DialogInterface dialog, int which) {
                            //UNISOC: add for bug1190072, java.lang.NullPointerException
                            if (getActivity() == null) {
                                return;
                            }
                            Intent intent = new Intent();
                            ...
                            String ids = getArguments().getString(
    ...
    /*
     * SPRD Bug 960972 bindContactData need contact in foreground
     * @{
     */
    ...
    private synchronized void tryRegisterImsListener() {
        //UNISOC: modify for bug1181955, delete ImsManagerEx.isVolteEnabledByPlatform() before registerforImsRegisterStateChanged
    ...
    private void unregistImsListener(){
        //UNISOC: modify for bug1181955, delete ImsManagerEx.isVolteEnabledByPlatform() before registerforImsRegisterStateChanged
        try {
            if (mIsImsListenerRegistered) {
            ...
            Log.e(TAG, "unregistImsListener: "+ e);
        }
    }
    public static boolean isVideoEnabledTmp = false;
    private final IImsRegisterListener.Stub mImsUtListenerExBinder = new IImsRegisterListener.Stub()
    {
        ...
        public void imsRegisterStateChange(boolean isRegistered) {
            int videoCapability = CallUtil.getVideoCallingAvailability(mContext);
            boolean isPresenceEnabled =
                        (videoCapability & CallUtil.VIDEO_CALLING_PRESENCE) != 0;
            boolean isVideoEnabled = (videoCapability & CallUtil.VIDEO_CALLING_ENABLED) != 0;
            if(isVideoEnabled != isVideoEnabledTmp && mContactData != null){
                isVideoEnabledTmp = isVideoEnabled;
                Log.d(TAG, "volte enable state change and refresh data.");
                // UNISOC: Bug 960972 bindContactData need contact in foreground
    ...
    private static class BlackColumns {
        ...
        public static final class BlackMumber implements BaseColumns {
            public static final Uri CONTENT_URI = Uri.parse("content://com.android.blockednumber/blocked");
            public static final String BLOCK_TYPE = "call_type";
            public static final String NAME = "name";
        }
    }

    private boolean isContactFilterable(ArrayList<String> phones) {
        phones.clear();
        /**
        ...
        for (ContentValues cv : cvs) {
            ...
            if (mimeType != null && mimeType.equals(Phone.CONTENT_ITEM_TYPE)) {
                ...
                /**
                 * SPRD:Bug 500476/935627 add a phone number contain extension number into blacklist, but this
        ...
        return phones.size() >= 1 ? true : false;
    }
    ...
    private boolean isCallFireWallInstalled() {
        ...
        try {
            getPackageManager().getPackageUid("com.sprd.firewall", 0);
            installed = true;
    ...
    private void addBlackItem(Menu menu, Contact mContactData) {
        menu.add(getString(R.string.menu_addToBlacklist));
        mBlackMenuItem = menu.getItem(menu.size() - 1);

        ArrayList<String> phones = new ArrayList<String>();
        mDisplayName = mContactData.getDisplayName();
        mOptionsMenuFilterable = isContactFilterable(phones);

        final class BlackListMenuItemsClickListener implements MenuItem.OnMenuItemClickListener {
            private ArrayList<String> lphones;

            public BlackListMenuItemsClickListener(ArrayList<String> phones) {
                lphones = phones;
            }
            ...
            public boolean onMenuItemClick(MenuItem item) {
                if (isContactBlocked()) {
                    onNotFilterRequested(lphones);
                } else {
                    onFilterRequested(lphones);
                }
                return true;
            }
        }
        mBlackMenuItem.setOnMenuItemClickListener(new BlackListMenuItemsClickListener(phones));
    }

    private void blackOrNot() {
        Log.d(TAG , "blackOrNot");
        if (mBlackMenuItem == null) {
            return;
        }
        // filter-able? (has any phone number)
        mBlackMenuItem.setVisible(mOptionsMenuFilterable);
        mBlackMenuItem.setEnabled(true);
        if (isContactBlocked()) {
            mBlackMenuItem.setTitle(getString(R.string.menu_deleteFromBlacklist));
        } else {
            mBlackMenuItem.setTitle(getString(R.string.menu_addToBlacklist));
        }
    }
    ...
    public boolean isContactBlocked() {
        ...
        ArrayList<ContentValues> contentValues = mContactData.getAllContentValues();
        for (ContentValues cv : contentValues) {
            String mimeType = cv.getAsString(Data.MIMETYPE);
            if (mimeType != null && mimeType.equals(Phone.CONTENT_ITEM_TYPE)) {
                String phone = cv.getAsString(Phone.NUMBER);
                if (!checkIsBlackNumber(phone)) {
                    return false;
                }
            }
    ...
    public boolean checkIsBlackNumber(String number) {
        /* UNISOC:androidq porting */
        TelephonyManager tm = (TelephonyManager) TelephonyManager.from(mContext);
        return tm.checkIsBlackNumber(mContext, formatPhoneNumber(number));
    }

    public void onNotFilterRequested(ArrayList<String> phones) {
        // remove all phones to blacklist
        for (int i = 0; i < phones.size(); i++) {
            String phone = phones.get(i);
            if (checkIsBlackNumber(phone)) {
                if (!deleteFromBlockList(phone)) {
                    Toast.makeText(this, getString(R.string.failed_removeFromBlacklist), Toast.LENGTH_SHORT).show();
                    return;
                }
            }
        }
        Toast.makeText(this, getString(R.string.success_removeFromBlacklist), Toast.LENGTH_SHORT).show();
    }

    public void onFilterRequested(final ArrayList<String> phones) {
        try {
            boolean success = true;
            boolean invalidPhoneNum = false;
            for (int i = 0; i < phones.size(); i++) {
                String origNumber = phones.get(i);
                String phone = PhoneNumberUtils.stripSeparators(origNumber);
                if (!checkIsBlackNumber(phone)
                        && !putToBlockList(phone, mDisplayName)) {
                    success = false;
                    invalidPhoneNum = !Telephony.Mms.isPhoneNumber(phone);
                    continue;
                }
            }
            if (success) {
                Toast.makeText(this, getString(R.string.success_addToBlacklist), Toast.LENGTH_SHORT).show();
            } else if (invalidPhoneNum) {
                Toast.makeText(this, getString(R.string.failed_wrong_phoneNumber), Toast.LENGTH_SHORT).show();
            } else {
                Toast.makeText(this, getString(R.string.failed_addToBlacklist), Toast.LENGTH_SHORT).show();
            }
        } catch (Exception e) {
            e.printStackTrace();
            Toast.makeText(this, getString(R.string.failed_addToBlacklist), Toast.LENGTH_SHORT).show();
    ...
    public boolean deleteFromBlockList(String phoneNumber) {
        ContentResolver cr = mContext.getContentResolver();
        String[] columns = new String[]{BlackColumns.BlackMumber._ID, BlockedNumberContract.BlockedNumbers.COLUMN_ORIGINAL_NUMBER,};
        ...
        try {
            if (cursor != null && cursor.moveToFirst()) {
                do {
                    original_number = cursor.getString(cursor.getColumnIndex(BlockedNumberContract.BlockedNumbers.COLUMN_ORIGINAL_NUMBER));

                    if (PhoneNumberUtils.compare(formatPhoneNumber(phoneNumber).trim(), original_number.trim())) {

                        result = cr.delete(BlackColumns.BlackMumber.CONTENT_URI, BlockedNumberContract.BlockedNumbers.COLUMN_ORIGINAL_NUMBER + "='" + original_number + "'", null);
                        break;
                    }
    ...
    public boolean putToBlockList(String phoneNumber, String name) {
        // UNISOC: Add for Bug1407880
        phoneNumber = formatPhoneNumber(phoneNumber);
        TelephonyManager tm = (TelephonyManager) TelephonyManager.from(mContext);
        int BLOCK_ALL = 7;   //block mms,phone,vt
        /* UNISOC: Add for bug 713431. @{ */
        if (!Telephony.Mms.isPhoneNumber(phoneNumber)) {
            return false;
        }
        /* @} */
        /* UNISOC: androidq porting */
        return tm.putToBlockList(mContext, phoneNumber, BLOCK_ALL, name);
    }

    /* UNISOC: Add for Bug1407880 Handle the case phone number contains semicolon or comma @{ */
    public String formatPhoneNumber(String number) {
        if (TextUtils.isEmpty(number)) {
            return "";
        }
        int specialCharIndex = -1;
        int waitIndex = number.indexOf(PhoneNumberUtils.WAIT);
        int pauseIndex  = number.indexOf(PhoneNumberUtils.PAUSE);
        if (waitIndex != -1) {
            specialCharIndex = waitIndex;
        } else if (pauseIndex != -1) {
            specialCharIndex = pauseIndex;
        }
        if (specialCharIndex != -1) {
            number = number.substring(0, specialCharIndex);
        }
        return number;
    }
    /* @} */
