...
public class ExpandingEntryCardView extends CardView {
    ...
    private View createEntryView(LayoutInflater layoutInflater, final Entry entry,
        ...
        if (!TextUtils.isEmpty(entry.getHeader())) {
            /* UNISOC:modify for bug 1059175 Adapt to Arbic  @{*/
            String mPhoneNumber = entry.getHeader().toString();
            if (ContactDisplayUtils.isRTL() && mPhoneNumber.contains("+")) {
                int phoneNumberLength = mPhoneNumber.length();
                int arPhoneIndex = mPhoneNumber.indexOf("+");
                if (phoneNumberLength - 1 > arPhoneIndex) {
                    String subNumber = mPhoneNumber.subSequence(arPhoneIndex, phoneNumberLength).toString();
                    String newNumber = BidiFormatter.getInstance().unicodeWrap(subNumber, TextDirectionHeuristics.LTR);
                    String numberAndString = mPhoneNumber.replace(subNumber, newNumber);
                    header.setText(numberAndString);
        ...
        if (!TextUtils.isEmpty(entry.getSubHeader())) {
            String subHeaderString = BidiFormatter.getInstance().unicodeWrap(entry.getSubHeader().toString(), TextDirectionHeuristics.LTR);
        ...
        /**SPRD Bug649290 Disable email app, Contacts should be prompt "No app was found to handle this action." when click email {@*/
