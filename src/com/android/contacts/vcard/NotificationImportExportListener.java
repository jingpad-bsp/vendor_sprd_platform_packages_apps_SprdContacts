...
public class NotificationImportExportListener implements VCardImportExportListener,
    ...
    //UNISOC: add for bug1137960, Bad notification for startForeground, invalid channel for service notification.
    private final static String LOG_TAG = "NotificationImportExportListener";
    ...
    public Notification onImportProcessed(ImportRequest request, int jobId, int sequence) {
        ...
        if (request.displayName != null) {
            // UNISOC: modify for bug1084337, Adapt to Arabic for vcf import Toast .
            displayName = ContactDisplayUtils.getFormatToastName(request.displayName);
    ...
    public Notification onImportParsed(ImportRequest request, int jobId, VCardEntry entry, int currentCount,
         ...
         * @{
         */
        if (ContactsNotificationChannelsUtil.needCreateDefaultChannel(mContext)) {
            Log.d(LOG_TAG, "Create Default Channel");
            ContactsNotificationChannelsUtil.createDefaultChannel(mContext);
        }
        /*
         * @}
    ...
    public void onImportFinished(ImportRequest request, int jobId, Uri createdUri) {
        ...
        mNotificationManager.notify(NotificationImportExportListener.DEFAULT_NOTIFICATION_TAG,
                jobId, notification);
        /**
         * SPRD: Bug715074 cancel the notification when finished import or cancel import  @{
         */
