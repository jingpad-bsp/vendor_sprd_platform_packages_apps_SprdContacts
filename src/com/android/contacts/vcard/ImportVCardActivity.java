...
public class ImportVCardActivity extends Activity implements ImportVCardDialogFragment.Listener {
    ...
    /**
     * SPRD:Bug 733003 Select a vcf file to import, in the selection of the accounts interface close
     * the contacts any permissions, then return to the authority to select the account after the split
     * screen, occasionally " contacts stop running".
     * @{
     */
    private boolean isVCardCacheThreadRunning;
    private static final String STATE_VCARDCACHETHREAD = "state_mVCardCacheThread";
    /**
     * @}
     */
    ...
    private class ImportRequestConnection implements ServiceConnection {
        ...
        public void onServiceConnected(ComponentName name, IBinder binder) {
            ...
            isVCardCacheThreadRunning = true;
    ...
    private class VCardCacheThread extends Thread
        ...
        public void run() {
            ...
            try {
                ...
                isVCardCacheThreadRunning = false;
    ...
    private String getDisplayName(Uri sourceUri) {
        ...
        /**
         * SPRD: Bug 595242 Vcf file name show incorrect.
    ...
    private Uri copyTo(final Uri sourceUri, String filename) throws IOException {
        ...
        try {
            ...
            //UNISOC: modify for bug1235734, monkey test, throw java.lang.NullPointerException.
            Log.e(LOG_TAG, "Failed to copy: " + e);
        } finally {
    ...
    private boolean isStorageUri(Uri uri) {
        ...
        * @{
         */
        //UNISOC: Bug1144019 can't open vcf files in Chrome downloads
        return uri != null && uri.toString().startsWith(STORAGE_VCARD_URI_PREFIX);
        /*
         * @}
         */
    ...
    protected void onCreate(Bundle bundle) {
         ...
         */
        if (bundle != null){
            isVCardCacheThreadRunning = bundle.getBoolean(STATE_VCARDCACHETHREAD);
        }
        /**
         * @}
         */
        ...
        if (sourceUri != null) {
            ...
            if (TextUtils.isEmpty(localTmpFileName)) {
                ...
                if (localTmpFileName == null) {
                    Log.e(LOG_TAG, "Cannot load uri to local storage.");
                    showFailureNotification(R.string.fail_reason_io_error);
                    //UNISOC: modify for bug1148628, if vcard import fail, enter contacts abnormally
                    finish();
                    return;
                }
    ...
    protected void onSaveInstanceState(Bundle outState) {
        ...
        outState.putBoolean(STATE_VCARDCACHETHREAD,isVCardCacheThreadRunning);
    }
    /**
     * @}
     */
