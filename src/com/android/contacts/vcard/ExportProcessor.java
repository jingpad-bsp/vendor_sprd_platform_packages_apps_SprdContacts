...
public class ExportProcessor extends ProcessorBase {
        ...
        try {
            ...
            */
            ...
            if (null != request.selectedContacts) {
                ...
                if(Constants.DEBUG) {
                    Log.d (LOG_TAG,"selectedContacts: "+sb.toString());
                }
                ...
                public void run() {
                    while (!Fcomposer.isAfterLast()) {
                        try {
                            ...
                            //UNISOC: modify for bug1382031, progress is stuck, fail to export contacts.
                            String data = Fcomposer.createOneEntry();
                     ...
                     while(true) {
                         try {
                             ...
                             try {
                                 ...
                                 String title = mService.getString(R.string.fail_reason_error_occurred_during_export,
                                 translatedErrorReason);
                                 /** UNISOC: add for bug1195157, The contact is exported as a VCF file and cannot be exported again after an export error. @{ */
                                 if (e.toString().contains("No space left on device")) {
                                     title = mService.getString(R.string.exporting_vcard_finished_title_storage_full);
                                 } else if (VCardComposer.NO_ERROR.equals(translatedErrorReason)) {
                                     title = mService.getString(R.string.fail_occurred_during_export);
                                 }
                                 doFinishNotification(title, null);
                                 mService.handleFinishExportNotification(mJobId, false);
                                 /** @｝ */
                                 return;
                             }
    ...
    private void doProgressNotification(Uri uri, int totalCount, int currentCount) {
        ...
        /**
         * SPRD: bug710214 869722 cancel export,the prompt is inaccurate
         * @{
         */
        String displayName = ExportVCardActivity.getOpenableUriDisplayName(mService,
                mExportRequest.destUri);
        /**
         * @}
         */
    ...
    private void doCancelNotification() {
        ...
        /**
         * SPRD: bug731722/872229, cancel export,the prompt is inaccurate
         * @{
         */
        String displayName = ExportVCardActivity.getOpenableUriDisplayName(mService,
                    mExportRequest.destUri);
