...
public class DrawerAdapter extends BaseAdapter {
    ...
    private boolean mGroupsHeaderClicked = false;
    ...
    /** SPRD: Bug693205 Add group feature for Contacts */
    private BaseDrawerItem mGroupHeader = null;
    ...
    private void initializeDrawerMenuItems() {
        ...
        // Group Header
        /** SPRD: Bug693205 Add group feature for Contacts */
        mGroupHeader = new MiscItem(R.id.nav_groups,
                R.string.menu_title_groups, R.drawable.quantum_ic_expand_more_vd_theme_24);
        // Account Header
    ...
    private void rebuildItemsList() {
         ...
         * @{
         */
        if (mGroupsHeaderClicked) {
            mItemsList.addAll(mGroupEntries);
        } else if (mGroupEntries.size() > 0){
            mItemsList.add(mGroupEntries.get(0));
        }
        /**
         * @}
         */
    ...
    private View getAccountItemView(AccountEntryItem item, View result, ViewGroup parent) {
        ...
        String currentAccountName = account.accountName;
        if (currentAccountName != null && mActivity != null && currentAccountName.equals("Phone")) {
            currentAccountName = mActivity.getResources().getString(R.string.label_phone);
        }
        textView.setText(currentAccountName);
        /* UNISOC: Bug1395057 If filterType changes, update activated to ensure that
         * the selected state of account view is right
         * @orig
        final boolean activated = account.equals(mSelectedAccount)
                && mSelectedView == ContactsView.ACCOUNT_VIEW;
        * @{ */
        boolean activated = account.equals(mSelectedAccount)
                && mSelectedView == ContactsView.ACCOUNT_VIEW;
        if (activated) {
            final ContactListFilter newFilter =  ContactListFilterController.getInstance(mActivity).getFilter();
            final ContactListFilter filter = getSelectedAccount();
    ...
    private View getDrawerItemView(BaseDrawerItem item, View result, ViewGroup parent) {
         ...
         * @{
         */
        if (item.id == R.id.nav_groups && mGroupsHeaderClicked) {
            iconView.setImageResource(R.drawable.quantum_ic_expand_less_vd_theme_24);
        } else {
            iconView.setImageResource(item.icon);
        }
        /**
     ...
     * @{
     */
    public void setGroupsHeaderClicked(Boolean isClicked) {
        if (mGroupsHeaderClicked == isClicked) {
            return;
        }
        mGroupsHeaderClicked = isClicked;
        notifyChangeAndRebuildList();
    }

    public boolean getGroupsHeaderClicked() {
        return mGroupsHeaderClicked;
    }
    /**
     * @}
     */
