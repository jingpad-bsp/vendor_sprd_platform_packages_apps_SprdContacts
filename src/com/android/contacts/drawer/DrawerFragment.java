...
public class DrawerFragment extends Fragment implements AccountsListener {
    ...
    private List<AccountInfo> mAccountsList;
    ...
    private final LoaderManager.LoaderCallbacks<Cursor> mGroupListLoaderListener =
            new LoaderManager.LoaderCallbacks<Cursor>() {
                ...
                public void onLoadFinished(Loader<Cursor> loader, Cursor data) {
                    ...
                    // SPRD: Bug731264 after rotate the screen, group disappear
    ...
    private final OnItemClickListener mOnDrawerItemClickListener = new OnItemClickListener() {
        ...
        public void onItemClick(AdapterView<?> parent, View v, int position, long id) {
             ...
             */
                ...
                mDrawerAdapter.setGroupsHeaderClicked(mDrawerAdapter.getGroupsHeaderClicked()
                        ? false : true);
                return;
            /**
             * @}
    ...
    public void onAccountsLoaded(List<AccountInfo> accounts) {
        ...
        /* UNISOC: add for bug1188302, the focus is still on sim account item after it was removed and then added @{ */
        final ContactListFilter filter = mDrawerAdapter.getSelectedAccount();
        if(filter != null && filter.filterType == ContactListFilter.FILTER_TYPE_ACCOUNT) {
            if (!filterAccountExists(filter)) {
                setNavigationItemChecked(ContactsView.ALL_CONTACTS);
            }
        }
        /* @} */

        // UNISOC: Bug712613 it still show contact and group of email account when disable the email
    ...
    /* UNISOC: add for bug1188302, the focus is still on sim account item after it was removed and then added @{ */
    private boolean filterAccountExists(ContactListFilter filter) {
        ...
        if (mAccountsLoaded && mGroupsLoaded) {
            ...
            while (iterator.hasNext()) {
                ...
                if (GroupUtil.isEmptyFFCGroup(groupListItem)
                        || !GroupUtil.isEnableGroup(groupListItem, mAccountsList)) {
                    iterator.remove();
                }
            }
