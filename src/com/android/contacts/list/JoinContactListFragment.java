...
public class JoinContactListFragment extends ContactEntryListFragment<JoinContactListAdapter> {
            ...
            switch (loader.getId()) {
                case DISPLAY_NAME_LOADER: {
                    if (data != null && data.moveToFirst()) {
                        mTargetContactName = data.getString(0);
                        /**
                         * SPRD: Bug723177 show a tip when there is no contact to be linked
                ...
                case JoinContactListAdapter.PARTITION_ALL_CONTACTS: {
                    if (data != null) {
                        ...
                        if (data.getCount() == 0) {
                            ...
                            showTargetContactName(mTargetContactName);
                        }
                        /**
                         * @}
                         */
    ...
    private void showTargetContactEmpty() {
        Activity activity = getActivity();
        TextView blurbView = (TextView) activity.findViewById(R.id.join_contact_blurb);
        String blurb = activity.getString(R.string.no_contact_can_be_linked);
        blurbView.setText(blurb);
    }
    /**
     * @}
