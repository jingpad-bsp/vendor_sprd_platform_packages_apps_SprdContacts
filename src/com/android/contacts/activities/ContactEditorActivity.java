...
import com.sprd.contacts.util.ActivityUtils;
...
public class ContactEditorActivity extends AppCompatContactsActivity implements
    ...
    public interface ContactEditor {
        ...
        void onSaveCompleted(boolean hadChanges, int saveMode, boolean saveSucceeded,
                             Uri contactLookupUri, Long joinContactId, int toastId);
    ...
    public void onCreate(Bundle savedState) {
        ...
        /**
         * SPRD:723140 add toast while can't save contact batch options
         *
         * @{
         */
         ...
         */
        /**
         * SPRD: Bug719300 it saves three contact numbers for sim contact @{
         */
        if (ActivityUtils.getInstance().activityHasStart(this)) {
            Activity mActivity = ActivityUtils.getInstance().getTopActivity();
            if (mActivity != null) {
                mActivity.finish();
            }
        }
        ActivityUtils.getInstance().addActivityToQueue(this);
        /**
         * @}
         */
    ...
    protected void onNewIntent(Intent intent) {
        ...
        if (Intent.ACTION_EDIT.equals(action)) {
            ...
            //SPRD: add for Bug718871, if there is no data in contacts, delete the contacts.
            boolean shouldDelete = intent.getBooleanExtra(ContactSaveService.EXTRA_SHOULD_DELETE, false);
            Log.d(TAG, "ContactEditorActivity : onNewIntent : shouldDelete = " + shouldDelete);
            if (shouldDelete) {
             ...
             * SPRD: Bug 768647 insert the sim contact,the toast is error when sim is loading
             * @{
             */
            int errorToastForSim =
                    intent.getIntExtra(ContactSaveService.EXTRA_SIM_LOADING_TOAST, 0);
            if (errorToastForSim != 0) {
                Toast.makeText(this, errorToastForSim, Toast.LENGTH_SHORT).show();
                finish();
                return;
            }
            /**
    ...
    public void onDestroy() {
        if (ActivityUtils.getInstance().activityHasStart(this)) {
            ActivityUtils.getInstance().removeActivity();
        }
        super.onDestroy();
    }
    /**
    ...
    public void onPickFromGalleryChosen() {
        Log.e(TAG,"onPickFromGalleryChosen");
