...
public class ContactEditorSpringBoardActivity extends AppCompatContactsActivity implements
    ...
    //SPRD: add for bug723129 limit the join contacts to 10
    ...
    private void toastErrorAndFinish() {
        //SPRD: Bug842998 it occured crash low probably when edit contact repeatly
    ...
    protected void onActivityResult(int requestCode, int resultCode, Intent data) {
        ...
        //SPRD: add for bug723129 limit the join contacts to 10
        if (requestCode == REQUEST_CODE_JOIN && data != null){
            ...
            if (contactJoinNum > MAX_JOIN_CONTACTS_NUMBER) {
                ...
                // SPRD: Bug900484 ContactEditorSpringBoardActivity is not finished when over max join number
                finish();
                return;
            }
        }
    ...
    /**
     * SPRD: Bug723129 limit the join contacts to 10 @{
     */
    private int getSelectedContactJoinNum(long contactId) {
        ...
        try {
            ...
            if (mResult != null){
                joinNum += mResult.rawContacts.size();
            }
        } finally {
            if (cursor != null) {
                cursor.close();
